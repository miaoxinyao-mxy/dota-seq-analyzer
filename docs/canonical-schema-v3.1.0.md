# DoTA-Seq Analyzer canonical result schema v3.1.0

This document is the frozen contract for canonical result schema v3.1.0. Every
...
One record per cell that passed the original cell/taxonomy filtering and entered
ASV typing. It therefore includes final cells and cells later excluded by ASV
status/quality, ASV-taxonomy consistency, or final ASV abundance filtering.

Candidate-cell records contain the same taxonomy, taxonomy-evidence, ASV-QC,
and sparse target structures available at their last evaluated stage, plus:

- `record_type`: `candidate_cell`.
- `final_filter.passed`: whether the cell is also present in `cells.jsonl`.
- `final_filter.stage`: `final`, `asv_status_quality`,
  `asv_taxonomy_conflict`, or `asv_abundance`.
- `final_filter.reason`: `null` for final cells; otherwise the existing ASV
  status/conflict reason or `low_asv_support`.

Cells removed before target background filtering retain their raw target counts.
For these records, `filtered_read_count` is `null` and `assignment_type` is
`not_evaluated`; no downstream scientific call is fabricated.

### `all_asvs.jsonl`

One record per candidate ASV represented among cells that reached final ASV
abundance filtering. Each record contains the ASV ID and core sequence,
`candidate_cell_count`, `candidate_cell_fraction`, and `final_filter`.
The fraction denominator is the number of cells with valid ASV assignments that
entered this filtering stage, not the total number of records in
`all_cells.jsonl`.

An ASV passes exactly when both configured conditions hold:

- candidate cell fraction >= `parameters.asv.final_minimum_cell_fraction`;
- candidate cell count >= `parameters.asv.final_minimum_cell_count`.

`cells.jsonl` and `asvs.jsonl` contain only records that pass this rule.
The companion `all_*.jsonl` files preserve excluded candidate provenance.
...
Canonical v3.1.0 deliberately does not store dominant taxonomy in ASV records.
...
- `barcode_funnel`: direct stage counts for `raw_unique_barcodes`,
  ...
  `after_asv_status_filter`, `asv_taxonomy_conflicts_removed`,
  `before_asv_abundance_filter`, `asv_abundance_filtered_cells`, and
  `final_cells`.
- `asv_summary`: `before_abundance_filter_count`,
  `filtered_out_asv_count`, and `final_asv_count`.
- `target_filtering_summary`: one record per target containing `target_name`,
  the existing target-background-filter counts, the additional
  `asv_abundance_excluded_positive_cells` and `final_positive_cells`, and
  final `retention_percent`.
...
- `asv`: `r1_start`, `r1_end`, `r2_start`, `r2_end`, `maximum_distance`,
  ...
  `final_minimum_cell_fraction`, `final_minimum_cell_count`
...
8. `barcode_funnel.final_cells` equals the number of final cell records, and
   `asv_summary.final_asv_count` equals the number of final ASV records.
9. Candidate/final filter decisions, candidate ASV fractions, and all
   candidate/final foreign keys and counts agree exactly across
   `all_cells.jsonl`, `all_asvs.jsonl`, `cells.jsonl`, and `asvs.jsonl`.
