...
def write_sub_arg_barcode_summary(
    ...
    # 2026-09-10: Write the unfiltered reconstruction table only when explicitly requested.
    # Reason: production consumers use the returned DataFrame and final working matrix instead.
    if sub_arg_barcode_summary_tsv is not None:
...
def main():
    ...
    parser.add_argument("--sub_arg_barcode_summary_tsv", type=str)
    ...
    parser.add_argument("--filtered_sub_arg_barcode_summary_tsv", type=str, default="tmp/cell_target_matrix.tsv")
    parser.add_argument("--filtered_stats_cells_per_sub_arg_tsv", type=str)
    ...
    # 2026-09-10: Materialize directories only for requested working outputs.
    # Reason: redundant reconstruction tables are no longer generated by default.
    ensure_output_directories(*[
        path for path in (
            args.sub_arg_barcode_summary_tsv,
